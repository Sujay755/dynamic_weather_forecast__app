...
const HourView = () => {
  ...
  return (
    <Base>
        <div>
            <div className={"w-full lg:w-2/4 justify-center mx-auto my-3" + (hours.loading ? ' visible' : ' hidden')}>
              <h1 className="font-bold text-lg">Loading...</h1>
            </div>
            <div className={"w-full lg:w-2/4 justify-center mx-auto my-3" + (hours.error ? ' visible' : ' hidden')}>
              <h1 className="font-bold text-lg">{hours.error}</h1>
            </div>
            <div className='flex flex-col text-left text-sm px-5 py-5 rounded-md bg-slate-100 w-full lg:w-1/2 mx-auto mt-5 mb-2'>
              <h1 className='font-bold text-lg sm:text-xl'>Hourly Weather - <span className='text-lg font-semibold'>{current.location.name}</span></h1>
              <h4 className='text-base sm:text-lg font-semibold'>As of {time}</h4>
              <hr className='mt-2'/>
            </div>
            <div>
              {hours.forecastHour.map((hour,index)=>(
